...
const ModalSix = ({service}) => {
  ...
  return(
    <div>
    < input type = "checkbox" id="booking-modal6" className = "modal-toggle" />
        <div className="modal">
            <div className="modal-box relative">
                <label htmlFor="booking-modal6"  className="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
               
               <form action="" onSubmit={handelmodal}  className='grid grid-cols-1 gap-5 mt-10'> 
               <input name='name' type="text" placeholder={service?.product6[0]?.name} className="input w-full font-bold text-black-500 input-bordered" />
               ...
               <input name='address'  placeholder='Enter Your Address' className="input w-full  font-bold text-black-500 input-bordered" />
                <button htmlFor="booking-modal6" className='btn w-full btn-accent font-bold'>Submit</button>
